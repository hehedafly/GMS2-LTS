...
if(ds_map_find_value(async_load,"type")=network_type_connect){
	if(ds_map_size(client_map)<playernum_limit){
		...
		if(log_id==-1){
			...
			var temp_time=date_datetime_string(date_current_datetime());
			file_text_write_string(log_id,"network record start------------------------------------------------")
			file_text_writeln(log_id);
			file_text_write_string(log_id,"network action record at "+temp_time);
			file_text_writeln(log_id);
		}
		if(log_id!=-1){
			file_text_write_string(log_id, "someone connecting");
			file_text_writeln(log_id);
		}
	}
}
	...
	if(ds_map_find_value(async_load,"type")=network_type_disconnect){
		...
		if(log_id!=-1){
			file_text_write_string(log_id, "someone disconnected");
			file_text_writeln(log_id);
		}
	}
}
if(ds_map_find_value(async_load, "type")=network_type_data){
	...
	if(log_id==-1){log_id=file_text_open_append(log_name);}
	if(log_id!=-1){

		file_text_write_string(log_id, "data collected");
		file_text_writeln(log_id);
	}
	...
	if(temp_status && temp_head_ls[| 1]==0 && ds_list_find_index(player_temp_uuid_ls, temp_head_ls[| 0])>=0){//pre_allo后客户端发来pre_uuid与username信息
		...
		if(log_id!=-1){
			file_text_write_string(log_id, "connecting: "+string([temp_head_ls[| 0], temp_head_ls[| 1], temp_head_ls[| 2]]));
			file_text_writeln(log_id);
		}
...
if(temp_status && temp_head_ls[| 1]==3){
	...
	if(temp_data_mark==0){//命令接收
		...
		if(log_id!=-1){
			file_text_write_string(log_id, "command_recived: "+string(temp_command)+" from "+string([temp_head_ls[| 0], temp_head_ls[| 1], temp_head_ls[| 2]]));
			file_text_writeln(log_id);
		}
...
//}
	if(log_id!=-1){
		file_text_write_string(log_id,"network record end------------------------------------------------")
		file_text_writeln(log_id);
		file_text_writeln(log_id);
		file_text_close(log_id);
		log_id=-1;
	}
